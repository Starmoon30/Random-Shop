// Generated by Selenium IDE

import org.junit.After;
import org.junit.Before;
import org.junit.Test;
import org.openqa.selenium.*;
import org.openqa.selenium.chrome.ChromeDriver;

import java.util.HashMap;
import java.util.Map;

public class Check_goods {
  private WebDriver driver;
  private Map<String, Object> vars;
  JavascriptExecutor js;
  @Before
  public void setUp() {
    System.setProperty("webdriver.chrome.driver", "C:\\Program Files\\Google\\Chrome\\Application\\chromedriver.exe");
    driver = new ChromeDriver();
    js = (JavascriptExecutor) driver;
    vars = new HashMap<String, Object>();
  }
  @After
  public void tearDown() {
    driver.quit();
  }
  @Test
  public void check() throws InterruptedException {
    driver.get("http://localhost:8090/");
    driver.manage().window().setSize(new Dimension(876, 677));
    driver.findElement(By.cssSelector("select")).click();
    {
      WebElement dropdown = driver.findElement(By.cssSelector("select"));
      dropdown.findElement(By.xpath("//option[. = 'Admin']")).click();
    }
    driver.findElement(By.cssSelector("option:nth-child(2)")).click();
    driver.findElement(By.cssSelector("input:nth-child(1)")).sendKeys("admin123");
    Thread.sleep(500);
    driver.findElement(By.cssSelector("input:nth-child(2)")).sendKeys("admin123");
    Thread.sleep(500);
    driver.findElement(By.cssSelector("input:nth-child(1)")).click();
    Thread.sleep(500);
    driver.findElement(By.cssSelector("button")).click();
    Thread.sleep(500);
    driver.findElement(By.xpath("//li[3]/ul/li")).click();
    Thread.sleep(500);
    driver.findElement(By.cssSelector(".el-card:nth-child(1) span")).click();
    Thread.sleep(500);
    driver.findElement(By.cssSelector(".el-button:nth-child(4) > span")).click();
    Thread.sleep(500);
    driver.findElement(By.xpath("//input")).click();
    Thread.sleep(500);
    driver.findElement(By.xpath("//input")).sendKeys("hat");
    Thread.sleep(500);
    driver.findElement(By.xpath("//div[2]/div/div/div/input")).sendKeys("58");
    Thread.sleep(500);
    driver.findElement(By.cssSelector(".el-button--primary > span")).click();
  }
}
