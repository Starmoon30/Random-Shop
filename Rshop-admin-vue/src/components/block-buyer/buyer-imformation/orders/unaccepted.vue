<template>
  <div>
    <el-scrollbar>
      <el-table :data="unacceptedTableData" class="custom-table-row" style="width: 100%">
    ...
    <el-pagination
      v-model:current-page="unacceptedPageNum"
...
const allData = ref([]);
// 定义表格数据的响应式变量
const unacceptedTableData = ref([]);
// 定义分页参数
const pageSize = ref(10);
const unacceptedPageNum = ref(1);
const unacceptedTotal = ref(0);
...
const fetchAllUsers = async () => {
  try {
    ...
    unacceptedTotal.value = allData.value.length; // 更新总数据量
...
// 分页函数
const paginate = (data) => {
  const startIndex = (unacceptedPageNum.value - 1) * pageSize.value;
  const endIndex = startIndex + pageSize.value;
  unacceptedTableData.value = data.slice(startIndex, endIndex);
};
